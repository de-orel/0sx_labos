...
Matrice::Matrice(int CLK_PIN, int DIN_PIN, int CS_PIN)
  : u8g2(U8G2_R0, CLK_PIN, DIN_PIN, CS_PIN, U8X8_PIN_NONE, U8X8_PIN_NONE) {
  clockPin = CLK_PIN;
  dataPin = DIN_PIN;
...
void Matrice::toggleOnOff() {
  ...
  if (millis() - lastUpdate >= 200 && isOn == false) {
    ...
    u8g2.sendBuffer();
  } else if (millis() - lastUpdate >= 200 && isOn == true) {
    lastUpdate = millis();
    isOn = false;
    u8g2.clearBuffer();
    u8g2.sendBuffer();
...
void Matrice::scrollString(String text) {
  ...
  if (millis() - lastScroll >= 70) {
    lastScroll = millis();
    static int x_pos = 8;

    const char* message = text.c_str();
    int largeur_txt = u8g2.getStrWidth(message);

    u8g2.clearBuffer();
    u8g2.drawStr(x_pos, 7, message);
    u8g2.sendBuffer();
